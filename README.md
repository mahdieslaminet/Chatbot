...
This project is an advanced chatbot that leverages modern technologies to create dynamic and efficient interactions. The main components of this chatbot are as follows:

Main Components

1. TTS (Text-to-Speech)
Converts the text generated by the chatbot into natural speech, enabling audio output for users.


2. STT (Speech-to-Text)
Converts the spoken input from users into text so that it can be processed by the chatbot, enabling voice-based interactions.


3. LANGDETECT (Language Detection)
Automatically detects the language of the user's input so that processing and responses are made in the appropriate language.


4. Text Language Detection
Identifies the language of the text input and allows the chatbot to support multiple languages for users.


5. ANONYMOUS SOUND
Uses voice anonymization techniques to maintain privacy and security for users in voice interactions.


6. Voice Conversion
Converts the voice of one person to that of another while preserving the original speech content, allowing for personalized voice responses.



**Key Features**

Multilingual Support: By using LANGDETECT and text language detection, the chatbot supports multiple languages.

Privacy Focus: Voice anonymization ensures the security of user data.
